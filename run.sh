...
while getopts 'hif' opt; do
  case "$opt" in
    ...
    i)
      ...
      curl -o $DB_DUMP https://raw.githubusercontent.com/navikt/lydia-api/49a61cec75d5acbb048a710071a8960cafbdd3df/scripts/db/lydia-api-container-db_localhost-2026_08_25_10_09_54-dump.sql
    ...
    f)
      ...
      docker rmi $(docker images | grep -E "lydia-api|lydia-radgiver-frontend" | cut -w -f2)
      ;;
...
then
  ...
  # kjør opp frontend
  pushd server && pnpm install && popd || exit
  cd client || exit
  pnpm install
  pnpm dev
fi
