...
export class Azure {
    ...
    constructor(
        ...
        tokenEndpoint: string = getEnvVar(
...
export class LydiaApi {
    ...
    constructor(uri = getEnvVar(miljøVariabler.lydiaApiUri)) {
        this.uri = uri;
...
const ifEnvVarExists = (variabelNavn: string) =>
    exists(getOptionalEnvVar(variabelNavn))
        ? Promise.resolve(getOptionalEnvVar(variabelNavn))
        : Promise.reject(`Miljøvariabelen ${variabelNavn} finnes ikke`);

const ifTheseEnvVarExists = (variabelNavn: string[]) =>
    allExists(variabelNavn.map((navn) => getOptionalEnvVar(navn)))
