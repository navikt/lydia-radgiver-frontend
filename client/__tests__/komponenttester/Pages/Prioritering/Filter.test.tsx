...
import { SykefraværsprosentVelger, Range } from "../../../../src/Pages/Prioritering/Filter/SykefraværsprosentVelger";
...
import { Eier, IAProsessStatusType } from "../../../../src/domenetyper/domenetyper";
...
const mockFylkerMedKommuner: FylkeMedKommuner[] = [
	{
		...
		kommuner: [
			{ nummer: "0301", navn: "Oslo", navnNorsk: "Oslo" },
		],
	},
	{
...
describe("Næringsgruppedropdown", () => {
	...
	test("rendrer label 'Bransjer og næringsgrupper'", () => {
		...
		expect(screen.getByText("Bransjer og næringsgrupper")).toBeInTheDocument();
	});
	...
	test("viser valgte næringsgrupper", () => {
		...
		expect(screen.getByText("62.010 - Programmeringstjenester")).toBeInTheDocument();
	});
...
describe("Filtervisning", () => {
	...
	const mockFiltervisning = {
		...
		state: {
			filterverdier: {
				...
				sektorer: [{ kode: "PRIVAT", beskrivelse: "Privat" }] as Sektor[],
	...
	test("rendrer Autosøk checkbox", () => {
		...
		expect(screen.getByRole("checkbox", { name: "Autosøk" })).toBeInTheDocument();
	});
	...
	test("toggler autosøk ved klikk på checkbox", () => {
		...
		expect(mockFiltervisning.oppdaterAutosøk).toHaveBeenCalledWith({ autosøk: true });
	});
	...
	test("aksepterer custom className", () => {
		...
		expect(container.querySelector(".custom-filter-class")).toBeInTheDocument();
	});
	...
	test("kaller oppdaterSektorer når sektor endres", async () => {
		...
		expect(mockFiltervisning.oppdaterSektorer).toHaveBeenCalledWith({ sektor: "PRIVAT" });
	});
	...
	test("kaller oppdaterIastatus når status endres", async () => {
		...
		expect(mockFiltervisning.oppdaterIastatus).toHaveBeenCalledWith({ iastatus: "VURDERES" });
	});
