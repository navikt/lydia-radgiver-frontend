...
describe("InnholdOppsett", () => {
	...
	test("rendrer checkbox-gruppe med tema-navn som legend", () => {
		...
		expect(screen.getByRole("group", { name: "Sykefraværsarbeid" })).toBeInTheDocument();
	});
	...
	test("viser checkboxer for hvert innhold", () => {
		...
		expect(screen.getByRole("checkbox", { name: "Undertema 1" })).toBeInTheDocument();
		expect(screen.getByRole("checkbox", { name: "Undertema 2" })).toBeInTheDocument();
		expect(screen.getByRole("checkbox", { name: "Undertema 3" })).toBeInTheDocument();
	});
	...
	test("viser checkbox som valgt når innhold er inkludert", () => {
		...
		expect(screen.getByRole("checkbox", { name: "Valgt undertema" })).toBeChecked();
		expect(screen.getByRole("checkbox", { name: "Ikke valgt undertema" })).not.toBeChecked();
	});
	...
	test("fjerner innhold fra plan når valgt checkbox klikkes", () => {
		...
		const checkbox = screen.getByRole("checkbox", { name: "Valgt undertema" });
		fireEvent.click(checkbox);
	...
	test("viser datepickers for inkludert innhold", () => {
		...
		expect(screen.getByRole("textbox", { name: /startdato for med datoer/i })).toBeInTheDocument();
		expect(screen.getByRole("textbox", { name: /sluttdato for med datoer/i })).toBeInTheDocument();
	});
	...
	test("viser ikke datepickers for ikke-inkludert innhold", () => {
		...
		expect(screen.queryByRole("textbox", { name: /startdato/i })).not.toBeInTheDocument();
		expect(screen.queryByRole("textbox", { name: /sluttdato/i })).not.toBeInTheDocument();
	});
...
describe("TemaInnholdVelger", () => {
	...
	test("rendrer checkbox-gruppe med tema-navn som legend", () => {
		...
		expect(screen.getByRole("group", { name: "Arbeidsmiljø" })).toBeInTheDocument();
	});
	...
	test("viser checkboxer for hvert undertema", () => {
		...
		expect(screen.getByRole("checkbox", { name: "Første undertema" })).toBeInTheDocument();
		expect(screen.getByRole("checkbox", { name: "Andre undertema" })).toBeInTheDocument();
	});

	test("kaller velgUndertemaer når checkbox velges", () => {
		const undertemaer = [createMockRedigertInnholdMal(1, "Undertema", false)];
	...
	test("viser feilmelding når visInnholdFeil er true", () => {
		...
		expect(
			screen.getByText(/Du må velge noe innhold for å opprette en samarbeidsplan/i)
		).toBeInTheDocument();
	});

	test("viser datepickers for inkluderte undertemaer", () => {
		const undertemaer = [createMockRedigertInnholdMal(1, "Med datoer", true)];
	...
	test("fjerner datoer når undertema fjernes fra planen", () => {
		const undertemaer = [createMockRedigertInnholdMal(1, "Undertema", true)];
