...
import { GyldigNesteHendelse, IASak } from "../../../../../src/domenetyper/domenetyper";
...
const createMockHendelse = (overrides: Partial<GyldigNesteHendelse> = {}): GyldigNesteHendelse => ({
...
describe("Knappecontainer", () => {
	test("rendrer children", () => {
		...
		expect(screen.getByRole("button", { name: "Test knapp" })).toBeInTheDocument();
	});
...
describe("hentÅrsakFraÅrsakType", () => {
	test("finner riktig årsak fra type", () => {
		...
		const årsak = hentÅrsakFraÅrsakType("NAV_IGANGSETTER_IKKE_TILTAK", hendelse);
...
describe("LenkeTilFanePåSamarbeid", () => {
	...
	test("rendrer lenke med riktig tekst", () => {
		...
		expect(screen.getByRole("link", { name: "Test lenke" })).toBeInTheDocument();
	});
	...
	test("lenke har riktig href med fane parameter", () => {
		...
		expect(link).toHaveAttribute("href", expect.stringContaining("fane=evaluering"));
	});
});
...
describe("LenkeTilBehovsvurderingFane", () => {
	...
	test("viser samarbeidsnavn som lenketekst", () => {
		...
		expect(screen.getByRole("link", { name: "Samarbeid A" })).toBeInTheDocument();
	});
	...
	test("viser 'Ukjent samarbeid' når samarbeid ikke finnes", () => {
		...
		expect(screen.getByRole("link", { name: "Ukjent samarbeid" })).toBeInTheDocument();
	});
});
...
describe("LenkeTilEvalueringsFane", () => {
	...
	test("viser samarbeidsnavn som lenketekst", () => {
		...
		expect(screen.getByRole("link", { name: "Samarbeid B" })).toBeInTheDocument();
	});
});
...
describe("LenkeTilSamarbeid", () => {
	...
	test("rendrer lenke med children", () => {
		...
		expect(screen.getByRole("link", { name: "Gå til samarbeid" })).toBeInTheDocument();
	});
...
describe("NesteSteg", () => {
	...
	test("viser riktig bekreftelsestekst for TILBAKE fra IKKE_AKTUELL", () => {
		...
		expect(screen.getByText(/setter saken tilbake til forrige status/i)).toBeInTheDocument();
	});
	...
	test("viser riktig tekst for TA_EIERSKAP_I_SAK når sak har eier", () => {
		...
		expect(screen.getByText(/nåværende eier blir automatisk fjernet/i)).toBeInTheDocument();
	});
