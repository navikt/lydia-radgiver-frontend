...
import BegrunnelserForIkkeKunne, { usePrettyType } from "../../../../src/Pages/Virksomhet/Samarbeid/EndreSamarbeidModal/BegrunnelserForIkkeKunne";
...
describe("BegrunnelserForIkkeKunne", () => {
	...
	test("mapper begrunnelse FINNES_SALESFORCE_AKTIVITET korrekt", () => {
		...
		expect(screen.getByText("Aktiviteter i Salesforce")).toBeInTheDocument();
	});
...
describe("VelgHandlingModal", () => {
	...
	test("rendrer modal med overskrift", () => {
		...
		expect(screen.getByRole("heading", { name: /Avslutt/i })).toBeInTheDocument();
	});
	...
	test("viser radioknapper for handlinger", () => {
		...
		expect(screen.getByRole("radio", { name: /fullført/i })).toBeInTheDocument();
		expect(screen.getByRole("radio", { name: /avbrutt/i })).toBeInTheDocument();
	});
