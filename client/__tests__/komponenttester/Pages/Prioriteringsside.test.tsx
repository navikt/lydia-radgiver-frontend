...
import { FilterverdiKategorier, Søkekomponenter } from "../../../src/util/analytics-klient";
...
describe("Paginering", () => {
	...
	test("rendrer sidetall", () => {
		render(
			<Paginering side={1} antallTreffPåSide={100} endreSide={mockEndreSide} />
		);
	...
	test("Forrige-knappen er aktivert på side 2+", () => {
		render(
			<Paginering side={2} antallTreffPåSide={100} endreSide={mockEndreSide} />
		);
		const forrigeKnapp = screen.getByRole("button", { name: /Forrige/i });
	...
	test("kaller endreSide med forrige side når Forrige klikkes", () => {
		render(
			<Paginering side={3} antallTreffPåSide={100} endreSide={mockEndreSide} />
		);
	...
	test("Neste-knappen er deaktivert når antallTreff < ANTALL_RESULTATER_PER_SIDE", () => {
		render(
			<Paginering side={1} antallTreffPåSide={50} endreSide={mockEndreSide} />
		);
	...
	test("viser korrekt sidetall", () => {
		render(
			<Paginering side={5} antallTreffPåSide={100} endreSide={mockEndreSide} />
		);
