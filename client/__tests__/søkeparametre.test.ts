...
describe("oversettelse fra søkeverdier til URL-parametre", () => {
    test("tomme søkeverdier resulterer i søkeparametre uten noen verdier", () => {
        ...
        expect(searchParams).toBe(
            "kommuner=&fylker=&neringsgrupper=&sykefraversprosentFra=&sykefraversprosentTil=&ansatteFra=&ansatteTil=&sorteringsnokkel=&iaStatus=&side=&kunMineVirksomheter="
        )
    })
    ...
    test("kommuner og fylker blir separert med komma som igjen blir escapet med %2C", () => {
        ...
        const searchParams = søkeverdierTilUrlSearchParams(søkeverdier).toString()
        expect(searchParams).toBe(
            `kommuner=0000%2C0001&fylker=03&neringsgrupper=&sykefraversprosentFra=&sykefraversprosentTil=&ansatteFra=&ansatteTil=&sorteringsnokkel=&iaStatus=&side=&kunMineVirksomheter=`
        )
