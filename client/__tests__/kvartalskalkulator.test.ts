...
describe("Testar logikk for utrekning av kvartal", () => {
        test('Finn rett kvartal for kvar månad', () => {
            expect(finnKvartalsNummer(new Date('January 01, 2023 16:00:00'))).toBe(1);
            expect(finnKvartalsNummer(new Date('February 01, 2023 16:00:00'))).toBe(1);
            ...
            expect(finnKvartalsNummer(new Date('August 01, 2023 16:00:00'))).toBe(3);
            expect(finnKvartalsNummer(new Date('September 01, 2023 16:00:00'))).toBe(3);
            expect(finnKvartalsNummer(new Date('October 01, 2023 16:00:00'))).toBe(4);
            expect(finnKvartalsNummer(new Date('November 01, 2023 16:00:00'))).toBe(4);
            expect(finnKvartalsNummer(new Date('December 31, 2023 16:00:00'))).toBe(4);
        })

        test("Får ut rett kvartal frå dato", () => {
            expect(datoTilKvartal(new Date('Januar 01, 2011 16:00:00')))
                .toMatchObject({kvartal: 1, årstall: 2011});
            expect(datoTilKvartal(new Date('December 31, 2023 23:59:00')))
        ...
        test("Sjekk at vi får rekna ut om saken var utan aktivitet i heile førre kvartal", () => {
            ...
            expect(aktivitetIForrigeKvartalEllerNyere(nå, forrigeKvartal)).toBe(true);
            expect(aktivitetIForrigeKvartalEllerNyere(nå, forriforrigeKvartal)).toBe(false);
