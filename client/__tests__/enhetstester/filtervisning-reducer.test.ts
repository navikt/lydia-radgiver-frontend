...
import { Filterverdier, ValgtSnittFilter } from "../../src/domenetyper/filterverdier";
...
const createTestFilterverdier = (overrides: Partial<Filterverdier> = {}): Filterverdier => ({
	fylker: [
	...
	sorteringsnokler: ["tapte_dagsverk", "sykefravarsprosent", "antall_personer"],
	statuser: ["IKKE_AKTIV", "VURDERES", "KONTAKTES", "KARTLEGGES", "VI_BISTÅR", "FULLFØRT", "IKKE_AKTUELL"],
...
describe("initialFiltervisningState", () => {
	test("har korrekte standardverdier", () => {
		...
		expect(initialFiltervisningState.sykefraværsprosent).toEqual({ fra: 0, til: 100 });
		expect(initialFiltervisningState.valgtSnittfilter).toBe(ValgtSnittFilter.ALLE);
		expect(initialFiltervisningState.antallArbeidsforhold).toEqual({ fra: 5, til: NaN });
...
describe("sammenliknFilterverdier", () => {
	...
	test("sammenligner side når ignorerSide er true (standard) - NB: invertert logikk", () => {
		...
		const stateA: FiltervisningState = { ...initialFiltervisningState, side: 1 };
		const stateB: FiltervisningState = { ...initialFiltervisningState, side: 5 };
...
describe("filterstateFraLokalstorage", () => {
	...
	test("returnerer lagret state når den er gyldig", () => {
		...
		const lagretState: Partial<FiltervisningState> = {
			...initialFiltervisningState,
			kommuner: [createKommune("0301", "Oslo")],
			valgteFylker: [createFylkeMedKommuner("03", "Oslo", [createKommune("0301", "Oslo")])],
		...
		localStorageMock.setItem("lokalFiltervisningState", JSON.stringify(lagretState));

		const result = filterstateFraLokalstorage(filterverdier);

		expect(result.kommuner).toEqual(lagretState.kommuner);
		expect(result.sykefraværsprosent).toEqual(lagretState.sykefraværsprosent);
	});
...
describe("Range og snittfilter verdier", () => {
	...
	test("ValgtSnittFilter har forventede verdier", () => {
		...
		expect(ValgtSnittFilter.BRANSJE_NÆRING_OVER).toBe("BRANSJE_NÆRING_OVER");
		expect(ValgtSnittFilter.BRANSJE_NÆRING_UNDER_ELLER_LIK).toBe("BRANSJE_NÆRING_UNDER_ELLER_LIK");
	});
});
