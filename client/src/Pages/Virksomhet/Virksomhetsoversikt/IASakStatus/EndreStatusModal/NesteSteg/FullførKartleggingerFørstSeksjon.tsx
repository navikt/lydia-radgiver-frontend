...
export function FullførKartleggingerFørstSeksjon({
    ...
    const { data: alleSamarbeidMedIkkeFullførteBehovsvurderinger } =
        useHentAlleSpørreundersøkelserAvType(
            sak,
            alleSamarbeid?.map((s) => s.id) || [],
            "BEHOVSVURDERING",
        );
    const { data: alleSamarbeidMedIkkeFullførteEvalueringer } =
        useHentAlleSpørreundersøkelserAvType(
            sak,
            alleSamarbeid?.map((s) => s.id) || [],
            "EVALUERING",
    ...
    return (
        <Modal.Body>
            <FullførFørstHeader
                ...
                alleSamarbeidMedIkkeFullførteEvalueringer={
                    alleSamarbeidMedIkkeFullførteEvalueringer
                }
            />
            <br />
            <BodyLong>
                For å gå videre må du fullføre eller slette kartleggingene som
                er påbegynt.
            </BodyLong>
            <br />
            {alleSamarbeidMedIkkeFullførteBehovsvurderinger.length > 0 && (
                <>
                    ...
                    <List as="ul" size="small">
                        {alleSamarbeidMedIkkeFullførteBehovsvurderinger.map(
                            (samarbeidId) => {
                                return (
            ...
            {alleSamarbeidMedIkkeFullførteEvalueringer.length > 0 && (
                <>
                    ...
                    <List as="ul" size="small">
                        {alleSamarbeidMedIkkeFullførteEvalueringer.map(
                            (samarbeidId) => {
                                return (
                                    <List.Item key={samarbeidId}>
                                        <LenkeTilEvalueringsFane
...
function FullførFørstHeader({
    ...
    alleSamarbeidMedIkkeFullførteEvalueringer?: number[];
    alleSamarbeidMedIkkeFullførteBehovsvurderinger?: number[];
}) {
    if (
        alleSamarbeidMedIkkeFullførteBehovsvurderinger &&
        alleSamarbeidMedIkkeFullførteEvalueringer &&
        ...
        return (
            <Heading level="2" size="medium">
                ...
                {alleSamarbeidMedIkkeFullførteBehovsvurderinger.length > 1
                    ...
                    : ""}{" "}
                og evaluering
                {alleSamarbeidMedIkkeFullførteEvalueringer.length > 1
                    ? "er"
                    : ""}{" "}
...
function useHentAlleSpørreundersøkelserAvType(
    iaSak: IASak,
    samarbeidsIder: number[],
    ...
    return {
        data: vurderingerResultater
            ...
            .flat()
            .filter(
                (v) => v?.status === "OPPRETTET" || v?.status === "PÅBEGYNT",
            )
            .map((v) => v?.samarbeidId)
