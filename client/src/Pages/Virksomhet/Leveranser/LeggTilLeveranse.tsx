...
export const LeggTilLeveranse = ({ iaSak, leveranserPerIATjeneste }: Props) => {
    ...
    const { data: iaTjenester, loading: lasterIATjenester } =
        useHentIATjenester();
    const { data: moduler } = useHentModuler();
    const [valgtIATjeneste, setValgtIATjeneste] = useState("");
    const [feiletForsøk, setFeiletForsøk] = useState(false);
    const ugyldigIATjeneste =
        valgtIATjeneste === "" ||
        valgtIATjeneste === undefined ||
        valgtIATjeneste === null;
    ...
    const filterBrukteIATjenester = (tjeneste: IATjeneste) => {
        if (moduler) {
            const modul = finnAktivModulFraIATjeneste(
                `${tjeneste.id}`,
                moduler,
            );

            for (
                let leveranseIndex = 0;
                leveranserPerIATjeneste &&
                leveranseIndex < leveranserPerIATjeneste.length;
                leveranseIndex++
            ) {
                ...
                for (
                    let index = 0;
                    index < leveranse.leveranser.length;
                    index++
                ) {
    ...
    return (
        <div>
            <Heading level="4" size="small">
                Legg til ny IA-tjeneste
            </Heading>
            {!brukerErEierAvSak && (
                <BodyShort spacing={true}>
                    Du må være eier av saken for å kunne legge til IA-tjenester
                </BodyShort>
            )}
            ...
            <Form onSubmit={(e) => e.preventDefault()}>
                <Select
                    ...
                    error={
                        ugyldigIATjeneste && feiletForsøk
                            ? "Du må velge en IA-tjeneste"
                            : undefined
                    }
                >
                    <option value="">
                        {lasterIATjenester && "Laster IA-tjenester..."}
                    </option>
                    {iaTjenester
                        ?.sort(iatjenesterStigendeEtterId)
                        .filter(filterBrukteIATjenester)
                        .map((tjeneste) => (
                            <option value={tjeneste.id} key={tjeneste.id}>
                                {tjeneste.navn}
                            </option>
                        ))}
                </Select>
