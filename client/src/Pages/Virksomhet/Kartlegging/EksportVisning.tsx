...
class pdfEksport {
	...
	constructor(targetRef: React.RefObject<HTMLDivElement>, eksportfilnavn: string, width: number) {
		this.pdf = new jsPDF('p', 'mm', 'a4');
		this.pageHeight = this.pdf.internal.pageSize.getHeight() - (pdfEksport.H_PADDING * 2);
		this.pageWidth = this.pdf.internal.pageSize.getWidth() - (pdfEksport.V_PADDING * 2);
	...
	private async addContent(canvas: HTMLCanvasElement, reducedVpadding = false, addHPadding = true) {
		...
		this.pdf.addImage(imgData, 'PNG', vPadding, this.position + hPadding, imgWidth, imgHeight);
	...
	private async addInlineContent(canvasL: HTMLCanvasElement, canvasR?: HTMLCanvasElement) {
		if (!canvasR) {
		...
		const imgHeight = Math.max(canvasL.height, canvasR.height) * this.pixelRatio;

		if (this.position + imgHeight > this.pageHeight) {
			this.addPage();
		}

		this.pdf.addImage(imgDataL, 'PNG', pdfEksport.V_PADDING, this.position + pdfEksport.H_PADDING, imgWidthL, imgHeight);
		this.pdf.addImage(imgDataR, 'PNG', imgWidthL + (pdfEksport.V_PADDING * 1.5), this.position + pdfEksport.H_PADDING, imgWidthR, imgHeight);
		this.position += imgHeight + pdfEksport.H_PADDING;
	}

	private async addHeader() {
		const headerTopCanvas = await html2canvas(this?.targetRef?.current?.childNodes[0]?.childNodes?.[0] as HTMLElement, { scale: 1 });
		const headerBottomCanvas = await html2canvas(this?.targetRef?.current?.childNodes[0]?.childNodes?.[1] as HTMLElement, { scale: 1 });
	...
	private async addBody() {
		...
		for (let i = 0; i < children.length; i++) {
			...
			if (child.tagName === 'DIV') {
				...
				const canvas = await html2canvas(child as HTMLElement, { scale: 1 });
				if (this.nextPageCantFitHeaderAndGraph(child, children[i + 1] as HTMLElement)) {
					this.addPage();
				}
	...
	private nextPageCantFitHeaderAndGraph(header: HTMLElement, graph: HTMLElement) {
		return this.position + header.clientHeight * this.pixelRatio + graph.clientHeight * this.pixelRatio > this.pageHeight;
	}

	private async addGraphs(child: Node) {
		for (let i = 0; i < child.childNodes.length; i += 2) {
			const canvasL = await html2canvas(child.childNodes[i] as HTMLElement, { scale: 1 });
			const canvasR = child.childNodes[i + 1] ? await html2canvas(child.childNodes[i + 1] as HTMLElement, { scale: 1 }) : undefined;
...
const EksportVisning = ({ erIEksportMode, setErIEksportMode, iaSak, kartlegging }: EksportVisningProps) => {
	...
	React.useEffect(() => {
		if (targetRef.current !== null && erIEksportMode && erLastet) {
			const pdfe = new pdfEksport(targetRef, eksportfilnavn, EXPORT_INTERNAL_WIDTH);
			pdfe.runExport().then(() => {
	...
	return (
		<>
			...
			<div style={{ height: 0, overflow: 'hidden', position: 'absolute' }}>
				<div ref={targetRef} style={{ display: erIEksportMode ? "block" : "none", width: EXPORT_INTERNAL_WIDTH }}>
