...
export default function BegrunnelserForIkkeKunne({
    ...
    begrunnelser?: KanIkkeGjennomføreBegrunnelse[];
    ...
    return (
        <Alert
            className={styles.begrunnelserAlert}
            variant={blokkerende ? "error" : "warning"}
        >
            <Heading spacing size="small" level="3">
                {blokkerende
                    ? `Samarbeidet kan ikke ${prettyType.uncapitalized}es:`
                    : `Er du sikker på at du ønsker å ${prettyType.uncapitalized}e?`}
            </Heading>
            <List>
...
function usePrettyBegrunnelser(
    begrunnelser?: KanIkkeGjennomføreBegrunnelse[],
): string[] | null {
    return React.useMemo(() => {
