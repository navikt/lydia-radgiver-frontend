...
export default function FullførteSamarbeid({
    iaSak,
    alleSamarbeid,
}: {
    ...
    const fullførteSamarbeid = alleSamarbeid
        ...
        ?.filter(
            ({ status }) =>
                status === IASamarbeidStatusEnum.enum.FULLFØRT ||
                status === IASamarbeidStatusEnum.enum.AVBRUTT,
        );

    if (
        !iaSak ||
        fullførteSamarbeid === undefined ||
        fullførteSamarbeid?.length === 0
    ) {
        return null;
    }
    ...
    if (fullførteSamarbeid.length < 3) {
        return (
            <>
                <ActionMenu.Divider />
                <ActionMenu.Group
                    label="Avsluttede samarbeid"
                    className={styles.samarbeidsDropdownAvsluttetListe}
                >
                    <FullførteSamarbeidListe
                        fullførteSamarbeid={fullførteSamarbeid}
                        iaSak={iaSak}
                    />
                </ActionMenu.Group>
            </>
...
function FullførteSamarbeidListe({
    fullførteSamarbeid,
    iaSak,
}: {
    fullførteSamarbeid: IaSakProsess[];
    iaSak: IASak;
}) {
    return fullførteSamarbeid.map((samarbeid) => (
        <Samarbeidslenke
            key={samarbeid.id}
            className={styles.samarbeidsDropdownFullførtItem}
            orgnr={iaSak.orgnr}
