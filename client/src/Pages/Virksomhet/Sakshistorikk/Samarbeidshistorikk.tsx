...
export default function Samarbeidshistorikk({
    ...
    const { sorterteSamarbeid, antallSkjulteRader } = React.useMemo(() => {
        ...
        const skalForkorte =
            kompakt && samarbeid.length > KOMPAKT_MAKS_ANTALL_RADER;

        return {
            sorterteSamarbeid: skalForkorte
                ? samarbeid.slice(0, KOMPAKT_MAKS_ANTALL_RADER - 1)
                : samarbeid,
            antallSkjulteRader: skalForkorte
                ? Math.max(
                      0,
                      samarbeid.length - (KOMPAKT_MAKS_ANTALL_RADER - 1),
                  )
                : 0,
        };
    }, [historikk.samarbeid]);
                    ...
                    <div className={styles.samarbeidrad} key={samarbeid.id}>
                        {lenke ? (
                            <InternLenke
                                ...
                                underline={false}
                                href={`/virksomhet/${orgnr}/sak/${samarbeid.saksnummer}/samarbeid/${samarbeid.id}`}
                            >
                                {samarbeid.navn}
                            </InternLenke>
                        ) : (
                            <BodyShort className={styles.medEllipse}>
                                {samarbeid.navn}
                            </BodyShort>
                        )}
                        <SamarbeidStatusBadge status={samarbeid.status} />
                        {(samarbeid.status ===
                            IASamarbeidStatusEnum.enum.FULLFØRT ||
                            samarbeid.status ===
                                IASamarbeidStatusEnum.enum.AVBRUTT) &&
