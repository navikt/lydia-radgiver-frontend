...
function genererTabellRader(
    historiskStatistikk: HistoriskStatistikk,
    visBransje: boolean,
) {
    let førsteKvartal = undefined as
        | { kvartal: number; årstall: number }
        | undefined;
    let sisteKvartal = undefined as
        | { kvartal: number; årstall: number }
        | undefined;
    ...
    for (let keyIndex = 0; keyIndex < keys.length; keyIndex++) {
        ...
        for (
            let index = 0;
            index < statistikkobjekt.statistikk.length;
            index++
        ) {
            const statistikk = statistikkobjekt.statistikk[index];
            if (
                førsteKvartal === undefined ||
                førsteKvartal.årstall > statistikk.årstall ||
                (førsteKvartal.årstall === statistikk.årstall &&
                    førsteKvartal.kvartal > statistikk.kvartal)
            ) {
                førsteKvartal = {
                    kvartal: statistikk.kvartal,
                    årstall: statistikk.årstall,
                };
            }

            if (
                sisteKvartal === undefined ||
                sisteKvartal.årstall < statistikk.årstall ||
                (sisteKvartal.årstall === statistikk.årstall &&
                    sisteKvartal.kvartal < statistikk.kvartal)
            ) {
                sisteKvartal = {
                    kvartal: statistikk.kvartal,
                    årstall: statistikk.årstall,
    ...
    while (aktivtÅrstall <= sisteKvartal.årstall) {
        while (
            (aktivtÅrstall === sisteKvartal.årstall &&
                aktivtKvartal <= sisteKvartal.kvartal) ||
            (aktivtÅrstall < sisteKvartal.årstall && aktivtKvartal <= 4)
        ) {
            rader.push({
                årstall: aktivtÅrstall,
                kvartal: aktivtKvartal,
                virksomhet: formaterProsent(
                    finnStatistikkverdi(
                        historiskStatistikk,
...
function finnStatistikkverdi(
    historiskStatistikk: HistoriskStatistikk,
    statistikktype: keyof HistoriskStatistikk,
    årstall: number,
    kvartal: number,
) {
    const funnetVerdi = historiskStatistikk[statistikktype].statistikk.find(
        (statistikk) =>
            statistikk.årstall === årstall && statistikk.kvartal === kvartal,
    );

    return funnetVerdi
        ? {
              erMaskert: funnetVerdi.maskert,
              prosent: funnetVerdi.sykefraværsprosent,
          }
        : undefined;
}
