...
export function SlettKartlegging({ orgnummer, onSuccess }: DeleteProps) {
	...
	const handleSubmit = async () => {
		setError(null);
		try {
			const result = await slettKartleggingNyFlyt(orgnummer, samarbeidId, spørreundersøkelseId);
			setResponse(result);
			onSuccess();
		} catch (e) {
	...
	return (
		<EndpointSection title="DELETE: slettKartleggingNyFlyt" response={response} error={error}>
			<div>
...
export function SlettSamarbeidsplan({ orgnummer, onSuccess }: DeleteProps) {
	...
	const handleSubmit = async () => {
		setError(null);
		try {
			const result = await slettSamarbeidsplanNyFlyt(orgnummer, samarbeidId);
			setResponse(result);
			onSuccess();
		} catch (e) {
			setError(e instanceof Error ? e.message : String(e));
	...
	return (
		<EndpointSection title="DELETE: slettSamarbeidsplanNyFlyt" response={response} error={error}>
			<div>
				<span>samarbeidId: </span>
				<input value={samarbeidId} onChange={(e) => setSamarbeidId(e.target.value)} />
			</div>
...
export function SlettSamarbeid({ orgnummer, onSuccess }: DeleteProps) {
	...
	return (
		<EndpointSection title="DELETE: slettSamarbeidNyFlyt" response={response} error={error}>
			<div>
				<span>samarbeidId: </span>
				<input value={samarbeidId} onChange={(e) => setSamarbeidId(e.target.value)} />
			</div>
